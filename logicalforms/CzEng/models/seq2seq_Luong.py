...
class EncoderRNN(nn.Module):
    def __init__(self, input_size,  hidden_size, voc_size, n_layers=1, dropout=0.1, fast_text=0, fast_text_vec = None):
        ...
        self.embedding = nn.Embedding(input_size, voc_size)
        self.fast_text = fast_text
        if fast_text == 1:
            fast_text_vec = torch.FloatTensor(fast_text_vec).cuda()
            self.embedding = self.embedding.from_pretrained(fast_text_vec)
            #self.lin_after_embedding = nn.Linear(300, voc_size)
        self.gru = nn.GRU(voc_size, hidden_size, n_layers, batch_first=True, dropout=self.dropout, bidirectional=True)
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        #if self.fast_text == 1:
        #    embedded = self.lin_after_embedding(embedded)
...
class Attn(nn.Module):
    def __init__(self, method, hidden_size, bi=1, soft=True, D= 0):
    ...
    def score(self, hidden, encoder_output):
        ...
        if self.method == 'dot':
            ...
            energy = self.attn(torch.cat((hidden.expand(encoder_output.shape[0],encoder_output.shape[1],-1), encoder_output), 2)) #energy: [Batch x Seq x hidden]
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, input_vocab_size, output_vocab_size, voc_size, n_layers=1, dropout=0.1, bi=0, fast_text=0, fast_text_vec = None):
        ...
        # Define layers
        self.embedding = nn.Embedding(input_vocab_size, voc_size)
        self.fast_text = fast_text
        if fast_text == 1:
            fast_text_vec = torch.FloatTensor(fast_text_vec).cuda()
            self.embedding = self.embedding.from_pretrained(fast_text_vec)
        ...
        self.gru = nn.GRU(voc_size, hidden_size, n_layers, dropout=dropout)
        self.concat = nn.Linear(hidden_size * 3, hidden_size)
        self.out = nn.Linear(hidden_size, output_vocab_size)

        # Choose attention model
        if attn_model != 'none':
            self.attn = Attn(attn_model, hidden_size, bi=1)
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs):
        ...
        embedded = embedded.view(1, batch_size, self.voc_size)  # embedded: [1 x Batch x voc]
        ...
        #Only at the first state
        if self.bi ==1 and last_hidden.shape[2] == 2*self.hidden_size:
            last_hidden = self.W_bi(last_hidden)
