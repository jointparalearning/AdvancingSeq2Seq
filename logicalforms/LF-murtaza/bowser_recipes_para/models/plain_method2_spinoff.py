...
class CopyDecoder(nn.Module):
    def __init__(self, which_attn_g, which_attn_c, generate_bahd, copy_bahd, vocab_size, embed_size, hidden_size, max_oovs=12, local_attn_cp=0, D=0, bi =0):
        ...
        self.attn = Attn(which_attn_g, hidden_size)
        ...
        if generate_bahd == 0:
            ...
            self.Wo = nn.Linear(hidden_size*2, vocab_size)
        ...
        if copy_bahd:
            ...
            self.Wc = nn.Linear(hidden_size, hidden_size*2)
            if which_attn_c == 'concat':
                self.Wc = nn.Linear(hidden_size*2, hidden_size*2)
                self.v = nn.Parameter(torch.randn(1,hidden_size*2,1))
            elif which_attn_c == 'location':
                self.Wc = nn.Linear(hidden_size, 1)
...
# =============================================================================
    ...
    def forward(self, input_idx, encoded, encoded_idx, prev_state, weighted, order, X_lengths):
        ...
        if self.copy_bahd:
            if self.which_attn_c == 'general':
                ...
                score_c = F.tanh(encoded.contiguous().view(-1,hidden_size))
                score_c = score_c.view(b,-1,hidden_size) # [b x seq x hidden_size]
                score_c = torch.bmm(score_c, state.unsqueeze(2)).view(b,-1) # [b x seq]
            elif self.which_attn_c == 'concat':
                concatted = torch.cat((rnn_output.unsqueeze(1).expand(encoded.shape[0],encoded.shape[1],encoded.shape[2]), encoded.contiguous()), 2)
        ...
        if self.local_attn_cp ==1 :
            ...
            #print(pt)
            s = torch.tensor([[j >= pt[i].item() - self.D and j <= pt[i].item() + self.D for j in range(X_lengths[i])] + [0.0]*(encoded.shape[1]- X_lengths[i]) for i in range(encoded.shape[0])]).cuda() #[batch x length of each sequence]
