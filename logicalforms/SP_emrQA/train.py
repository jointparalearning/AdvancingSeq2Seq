...
def train(num_epochs, args):
    ...
    if args.cos_hinge == 1:
        ...
        def hinge_cos_loss(anchors, positives, negatives, margin):
            ...
            for i in range(anchors.shape[0]):
                ...
                total_loss += max(temp_coss_loss(anchor.view(1,-1), positive.view(1,-1), ones) - temp_coss_loss(anchor.view(1,-1), negative.view(1,-1), ones) + margin, torch.tensor(0.0).cuda())
    ...
    #load model 
    if args.bert_sent ==1:
        bert_encoded = cPickle.load(open('data/bert_word_sh2_spl3.p', 'rb'))
        bert_hidden = cPickle.load(open('data/bert_sentence_sh2_spl3.p', 'rb'))
        training_encoded = bert_encoded['training_sampled'][0]
        training_hidden = bert_hidden['training_sampled'][0]
        
        validation_encoded =bert_encoded['validation_sampled'][0]
        validation_hidden = bert_hidden['validation_sampled'][0]
