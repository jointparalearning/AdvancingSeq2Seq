...
def train(num_epochs, args):
    ...
    if m_ver in [0,2,4]: #DID UNTIL HERE!
        ...
        decoder = CopyDecoder('general', 'general', 0, 1, vocab_size, embed_dim, hidden_dim)
        #AUTOdecoder = AutoDecoder(AUTOhidden_dim, vocab_size)
        AUTOdecoder = LuongAttnDecoderRNN('general', hidden_dim, vocab_size)
    ...
    for epoch in epochs:
        ...
        while (batch_num) * batch_size < len(training_sampled):
            ...
            ###AUTO DECODER
            AUTOdecoder_hidden, AUTOdecoder_input, AUTOtarget_tensor, di = encoded[:, -1, :], torch.tensor([SOS_token]*batch_size, device=device), sentence_vectors, 1
            use_teacher_forcing = True if random.random() < teacher_forcing_prob else False
