...
class VAEEncoderRAW(nn.Module):

    def __init__(self, hidden_dim, vocab_size, latent_dim):
        super(VAEEncoderRAW, self).__init__()
        self.hidden_dim = hidden_dim
        self.latent_dim = latent_dim
        self.embedding =  nn.Embedding(vocab_size, hidden_dim, padding_idx=0)
        self.gru = nn.GRU(hidden_dim+1, hidden_dim, batch_first=True,
            bidirectional=True)
    ...
    def forward(self, sentence2idxvec, X_lengths):
        ...
        embeds = torch.cat([embeds, binary_vectors.view(binary_vectors.shape[0], binary_vectors.shape[1], -1)], dim=2)
        embeds = torch.nn.utils.rnn.pack_padded_sequence(embeds, X_lengths, batch_first=True)
        lstm_out, hidden = self.gru(embeds)
        hidden_ec = hidden
        hidden = hidden.view(batch_size, -1) #[batch_size x hidden_dim*2]
        
        lstm_out, _ = torch.nn.utils.rnn.pad_packed_sequence(lstm_out, batch_first=True)
        
        return lstm_out, hidden_ec
...
class AutoDecoder(nn.Module):
    ...
    def forward(self, input, hidden, order):
        if order == 0:
            ...
            z = vae_to_var(torch.randn([batch_size, self.latent_dim]))
            z = z * std + mean
...
class CopyDecoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, latent_dim, max_oovs=12):
        ...
        self.gru = nn.GRU(input_size=embed_size+hidden_size*2+1,
            hidden_size=hidden_size, batch_first=True)
        self.max_oovs = max_oovs # largest number of OOVs available per sample

        # weights
        self.Ws = nn.Linear(hidden_size*2+1, hidden_size) # only used at initial stage
        self.Wo = nn.Linear(hidden_size, vocab_size) # generate mode
        self.Wc = nn.Linear(hidden_size*2+1, hidden_size) # copy mode
    ...
    def forward(self, input_idx, encoded, encoded_idx, prev_state, weighted, order, train_mode):
        ...
        encoded = torch.cat([encoded, binary_vectors.view(binary_vectors.shape[0], binary_vectors.shape[1], -1)], dim=2)

        # 0. set initial state s0 and initial attention (blank)
        if order==0:
            prev_state = self.Ws(encoded[:,-1])
            #prev_state = self.latent2hidden(prev_state)
        ...
        #print("prev stat ehspae:", prev_state.shape)
