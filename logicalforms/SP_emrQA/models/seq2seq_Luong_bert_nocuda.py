...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, X_lengths = None):
        ...
        if self.method in ['dot', 'general', 'concat']:
            ...
            if USE_CUDA:
                attn_energies_3 = attn_energies_3
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, output_size, n_layers=1, dropout=0.1, bi=0, bert_sent=0, BERTFinetune_embedding=None):
...
# =============================================================================
        ...
        if not(BERTFinetune_embedding is None):
            bert_vectors = torch.FloatTensor(BERTFinetune_embedding)
            self.embedding = self.embedding.from_pretrained(bert_vectors)
            self.bert_lin = nn.Linear(768, hidden_size*2)
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs):
        ...
        if not(self.BERTFinetune_embedding is None):
            embedded = self.bert_lin(embedded)
